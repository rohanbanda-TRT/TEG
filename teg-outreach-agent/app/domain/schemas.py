...
class CustomHtmlSection(BaseModel):
    """A custom HTML section generated by Claude for content not in the standard frontend."""
    section_id: str  # unique identifier for the section
    title: str  # display title for the section
    html_content: str  # the HTML content
    position: str  # where to insert: "after_hero", "before_pains", "after_proof", "before_closing"
    confidence: str = "hypothesis"  # evidence confidence: verified, inferred, hypothesis, requires_confirmation
...
class Proposal(BaseModel):
    ...
    custom_html_sections: list[CustomHtmlSection] = Field(default_factory=list)
    # Populated only when a fresh background deep-research pass exists for
    # this company (app/research/deep.py) — both empty when it doesn't, so
    # the page renders identically to before deep research existed. See
    # docs/superpowers/specs/2026-09-09-background-deep-research-design.md.
    # company_standing is assembled deterministically from stored
    # DeepFindings (no LLM authorship — see ProposalAgent._company_standing),
    # the same "state facts plainly, never invent" discipline every other
    # forced field in this schema already follows.
    company_standing: str = ""
    teg_fit_points: list[str] = Field(default_factory=list)
